...
const loadFavorites = async () => {
    const favorites = JSON.parse(localStorage.getItem('favoriteCameras')) || [];
    ...
    try {
        // Fetch all cameras to match codes (simplified approach, ideally backend would support fetching by IDs)
...
const removeFavorite = (code) => {
    let favorites = JSON.parse(localStorage.getItem('favoriteCameras')) || [];
    favorites = favorites.filter(fav => fav !== code);
    localStorage.setItem('favoriteCameras', JSON.stringify(favorites));
